...
const OwnerTableWithChairs = ({ table, big }: { table: tableType, big: boolean }) => {
   const { id: tableId, seats: seatCount, isAvailable, tableNumber, _count: { reservationHistory: pastReservations } } = table
   ...
   return (
      <HoverCard openDelay={100} closeDelay={100} open={big ? false : undefined}>
         <HoverCardTrigger asChild>
            <div className={"relative select-none cursor-pointer " + (big ? "w-130 h-130 " : "w-50 h-45 ")} onClick={!big ? () => { navigate(`/table/${tableId}`); scrollTo({ behavior: "smooth", top: 0 }) } : () => { }}>
               {/* Table */}
         ...
         <HoverCardContent side="top" asChild={true}>
            <div className={"!w-50 font-[Rubik] rounded-xl p-5 border-1 " + (isAvailable ? "border-green-400" : "border-red-300")}>
               ...
               <div className="text-sm">
                  ...
                  <div className="flex justify-between"><p>All-Time Bookings: </p>{pastReservations}</div>
               </div>
            </div>
         </HoverCardContent>
      </HoverCard>
...
export default OwnerTableWithChairs