  ...
  if (error instanceof AxiosError) {
    if (error.response) {
      if (error.response.status == 401) {
        console.log("Tampered JWT: ", error)
        toast.warning("Phishy Credentials", { description: "Login Again.", id: loadingID })
        return;
      }
      console.log("User is not allowing third party cookies: ", error)
      toast.warning("You need to enable third-party cookies", { description: "Change your cookie preferences to login.", id: loadingID })
    }
